 ...
 *
 * Core formula:
 *   Active Clients ≈ (F × VR × LR × CR × P) / Churn
 *
 *   F     = total followers (primary platform)
 *   VR    = visibility rate — % of followers reached per monetization post
 *   LR    = lead rate — % of reached who enter funnel
 *   CR    = conversion rate — % of leads who become paying subscribers
 *   P     = monetization posts/CTAs per month
 *   Churn = monthly % of paying subscribers lost
 *
 * Simplified: Active Clients ≈ F × 0.00675
 *   (VR=15%, LR=3%, CR=8%, P=15, Churn=8%)
 *
 * Benchmarks:
 *   Low performers:    0.2% - 0.4% of followers
 *   Average creators:  0.5% - 1.0% of followers
 *   Strong operators:  1.0% - 3.0% of followers
 */
    ...
    <div>
      ...
      <div style={{ textAlign: "center", padding: "32px 20px 28px", marginBottom: 24, background: "linear-gradient(180deg, #0f0806 0%, #080604 100%)", borderRadius: 6, border: "1px solid #1e1b1733" }}>
        ...
        <div style={{ fontSize: 48, fontWeight: 200, color: "#7A0E18", letterSpacing: "-0.03em", lineHeight: 1.1 }}>
          {fmt(mod.monthlyRevenue)}
        </div>
        <div style={{ fontSize: 11, color: "#2a2720", marginTop: 6 }}>
          /month &middot; {mod.activeClients} active clients &middot; {platform} {F.toLocaleString()} followers
        </div>
        <div style={{ fontSize: 10, color: "#2a2720", marginTop: 3 }}>
          Simplified estimate: {simplified} active clients (F &times; 0.00675)
        </div>
      </div>
      ...
      <div style={{ padding: "18px 22px", borderRadius: 4, background: "#060503", border: "1px solid #141210", marginBottom: 16 }}>
        ...
        <p style={{ fontSize: 13, color: "#E2E4DF", margin: "0 0 12px", fontFamily: "monospace", letterSpacing: "0.02em" }}>
          Active Clients = (F &times; VR &times; LR &times; CR &times; P) / Churn
        </p>
        ...
        <div style={{ borderTop: "1px solid #141210", paddingTop: 12, marginTop: 4 }}>
          ...
          <p style={{ fontSize: 13, color: "#E2E4DF", margin: "0 0 6px", fontFamily: "monospace" }}>
            Active Clients &asymp; F &times; 0.00675 = {F.toLocaleString()} &times; 0.00675 = <span style={{ color: "#7A0E18" }}>{simplified}</span>
          </p>
          <p style={{ fontSize: 10, color: "#2a2720", margin: 0 }}>
            Based on: VR=15%, LR=3%, CR=8%, P=15 posts/mo, Churn=8%
          </p>
        </div>
      </div>
...
export default function OfferBuilder() {
  ...
  const scrape = async () => {
    ...
    try {
      ...
      // Capture structured profile data (biography, externalUrls, etc.) from Apify
      if (data.profiles) profiles._profiles = data.profiles;
      return profiles;
    } catch { return {}; }
  };
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "28px 20px 80px" }}>
        ...
        {result && <div ref={ref}>
          <div style={{ marginBottom: 28, padding: "28px 24px", borderRadius: 6, background: "#080604", border: "1px solid #141210" }}>
            ...
            {(() => {
              const promiseMatch = (result.raw || "").match(/(?:Core Promise|Promessa Central|Promessa Principal|B\.\s*(?:Core Promise|Promessa))[:\s]*"?([^"\n]+)"?/i);
              if (promiseMatch) return <p style={{ fontSize: 14, color: "#7A0E18", margin: 0, fontWeight: 400, lineHeight: 1.5, maxWidth: 560 }}>{promiseMatch[1].trim().replace(/^[""]|[""]$/g, "")}</p>;
              return null;
            })()}
          </div>
          ...
          {result.scraped && Object.keys(result.scraped).length > 0 && (
            <div style={{ marginBottom: 16, padding: "8px 14px", borderRadius: 3, background: "#7A0E1808", border: "1px solid #141210", fontSize: 11, color: "#4a4840" }}>
              Social intelligence gathered: {Object.keys(result.scraped).filter(k => k !== "_profiles").join(", ")}
            </div>
          )}

          {result.scraped?._profiles?.instagram && (
            <div style={{ marginBottom: 20, padding: "18px 22px", borderRadius: 6, background: "#080604", border: "1px solid #141210" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
                ...
                {result.scraped._profiles.instagram.verified && <span style={{ fontSize: 8, fontWeight: 600, color: "#3b82f6", letterSpacing: "0.06em", padding: "1px 5px", borderRadius: 2, border: "1px solid #3b82f633", textTransform: "uppercase" }}>Verified</span>}
              </div>
              {result.scraped._profiles.instagram.biography && (
                <p style={{ fontSize: 13, color: "#c5c3be", margin: "0 0 12px", lineHeight: 1.6, whiteSpace: "pre-line" }}>
                  {result.scraped._profiles.instagram.biography}
                </p>
              )}
              {result.scraped._profiles.instagram.externalUrls?.length > 0 && (
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {result.scraped._profiles.instagram.externalUrls.map((url, i) => (
              ...
              <div style={{ display: "flex", gap: 16, marginTop: 12, paddingTop: 10, borderTop: "1px solid #141210" }}>
                {result.scraped._profiles.instagram.followersCount != null && (
                  <span style={{ fontSize: 11, color: "#6b6860" }}>Followers: <span style={{ color: "#E2E4DF", fontWeight: 600 }}>{result.scraped._profiles.instagram.followersCount.toLocaleString()}</span></span>
                )}
                {result.scraped._profiles.instagram.postsCount != null && (
                  <span style={{ fontSize: 11, color: "#6b6860" }}>Posts: <span style={{ color: "#E2E4DF", fontWeight: 600 }}>{result.scraped._profiles.instagram.postsCount.toLocaleString()}</span></span>
                )}
              </div>
            </div>
          )}
