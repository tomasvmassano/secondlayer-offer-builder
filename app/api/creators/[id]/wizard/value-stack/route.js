...
${extraInstruction ? `## ADDITIONAL INSTRUCTION\n${extraInstruction}\n\n` : ''}Return ONLY the JSON object per the schema in the system prompt.`;
  ...
  if (!parsed) {
    if (retryCount < 1) return runValueStack(apiKey, creator, retryCount + 1, 'Your previous response was not parseable JSON. Return ONLY a JSON object — no prose, no markdown fences.');
  ...
  if (!validation.valid) {
    if (retryCount < 1) {
      ...
      if (retryResp.ok) {
        ...
        if (retryParsed) {
          if (retryParsed.value_stack && client.target_price) retryParsed.value_stack.actualPrice = client.target_price;
          const retryValidation = validateValueStack(retryParsed);
          if (retryValidation.valid) return enrich(retryParsed, modules.length, client.target_price, retryValidation.warnings, retryCount + 1);
